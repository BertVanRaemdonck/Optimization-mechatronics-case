classdef  MetaVar < casadi.IndexAbstraction
    %METAVAR 
    %
    %   = METAVAR()
    %
    %
  methods
    function v = attribute(self)
      v = casadiMEX(1141, self);
    end
    function v = n(self)
      v = casadiMEX(1142, self);
    end
    function v = m(self)
      v = casadiMEX(1143, self);
    end
    function v = type(self)
      v = casadiMEX(1144, self);
    end
    function v = count(self)
      v = casadiMEX(1145, self);
    end
    function v = i(self)
      v = casadiMEX(1146, self);
    end
    function v = extra(self)
      v = casadiMEX(1147, self);
    end
    function self = MetaVar(varargin)
    %METAVAR 
    %
    %  new_obj = METAVAR()
    %
    %
      self@casadi.IndexAbstraction(SwigRef.Null);
      if nargin==1 && strcmp(class(varargin{1}),'SwigRef')
        if ~isnull(varargin{1})
          self.swigPtr = varargin{1}.swigPtr;
        end
      else
        tmp = casadiMEX(1148, varargin{:});
        self.swigPtr = tmp.swigPtr;
        tmp.swigPtr = [];
      end
    end
    function delete(self)
      if self.swigPtr
        casadiMEX(1149, self);
        self.swigPtr=[];
      end
    end
  end
  methods(Static)
  end
end
